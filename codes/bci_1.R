...
###WARNING! Lengthy code! Each parameter combination requires 2-3 minutes of runtime. There
...
pcaplot=pca@loadings%>%as_tibble()
nutnames=names(data%>%select(Al:water))

ggplot()+
geom_hline(yintercept = 0, lty = 2, color = "grey", alpha = 0.9) +
geom_vline(xintercept = 0, lty = 2, color = "grey", alpha = 0.9) +
geom_segment(data=pcaplot,aes(x=0,xend=PC1,y=0,yend=PC2),
             arrow=arrow(length=unit(0.025, "npc"), type = "open"))+
geom_text(data = pcaplot, 
          aes(x = PC1*1.15, y =  PC2*1.15,
          label = nutnames), 
...
ggplot()+
  geom_hline(yintercept = 0, lty = 2, color = "grey", alpha = 0.9) +
  geom_vline(xintercept = 0, lty = 2, color = "grey", alpha = 0.9) +
  geom_segment(data=pcaplot,aes(x=0,xend=PC2,y=0,yend=PC3),
               arrow=arrow(length=unit(0.025, "npc"), type = "open"))+
  geom_text(data = pcaplot, 
            aes(x = PC2*1.15, y =  PC3*1.15,
                label = nutnames), 
            check_overlap = F, size = 3) +
  xlab("PC 2") + 
  ylab("PC 3")
ggplot()+
  geom_hline(yintercept = 0, lty = 2, color = "grey", alpha = 0.9) +
  geom_vline(xintercept = 0, lty = 2, color = "grey", alpha = 0.9) +
  geom_segment(data=pcaplot,aes(x=0,xend=PC1,y=0,yend=PC3),
               arrow=arrow(length=unit(0.025, "npc"), type = "open"))+
  geom_text(data = pcaplot, 
            aes(x = PC1*1.15, y =  PC3*1.15,
                label = nutnames), 
            check_overlap = F, size = 3) +
  xlab("PC 1") + 
  ylab("PC 3") 
...
kmeans_dat=bci_dat%>%
  filter(census==7)%>%
  inner_join(trait_data_w,relationship = "many-to-many")%>%
  inner_join(
    cluster_data%>%
      filter(census==7,d_cutoff==20)%>%
      select(sp,group),relationship = "many-to-many")
...
#Warning: Long computation time! Reduce the number of seeds to test execution
k = 
  future_pmap_dfr(
    expand_grid(
      groups = 2:10, 
...
k4=kmeansW(trdat%>%select(PC1,PC2),centers=4,weight = trdat$abun,nstart=100)

trdat%<>%mutate(kmeans=k4$cluster)%>%mutate(kmeans=as.factor(kmeans))
...
pca_vital=
  ...
  select(sp, trait, standardized) %>%
  pivot_wider(names_from = trait, values_from = standardized)%>%
  #mutate(RGR95SAP=RGR95SAP+rnorm(length(RGR95SAP),0,0.5))%>%
  select(-sp)%>%
  pca(nPCs=4,method = 'ppca', scale = 'none', center = FALSE)
...
trdat_group=trdat%>%select(-abun)%>%
  inner_join(abuns1,by=c('sp'))%>%
  mutate(abun=ceiling(abun))%>%
  inner_join(cluster_data%>%
               filter(census==7,d_cutoff==20)%>%
               select(sp,group)%>%unique())

trdat_group=trdat_group[rep((1:nrow(trdat_group)),trdat_group$abun),]

trdat_group%<>%select(group,vital,leaf,seed,wood,size)%>%
  pivot_longer(cols=vital:size,names_to = "trait",values_to = "value")

cmpr=list(c(1,2),c(1,3),c(1,4),c(2,3),c(2,4),c(3,4))

ggboxplot(trdat_group, x = "group", y = "value",
          color = "group")+
...
kde_kmeans=kde_kmeans%>%
  inner_join(
    kde_kmeans%>%
      group_by(x,y)%>%
      summarize(tot=sum(density))%>%
      ungroup())%>%
  mutate(density=density/tot)%>%
  select(-tot)
...
#Leaf elemental concentration traits analysis
...
pca_data=trait_data_w%>%
  select(-sp)%>%
  pcaMethods::pca(method='ppca',scale='none',center=FALSE)
...
kdash=kmeansW(trdat%>%select(AL:ZN),centers=3,weight = trdat$abun,nstart=100)
...
trdat%>%
  pivot_longer(cols=AL:ZN,names_to = 'Element',values_to = 'Concentration')%>%
